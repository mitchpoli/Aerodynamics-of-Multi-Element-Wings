...
% Seleziona il file immagine (PNG/JPG/TIFF) e ottieni anche il path
[filename, pathname] = uigetfile({'*.png;*.jpg;*.tiff','Image files (*.png, *.jpg, *.tiff)'}, ...
                                 'Select an image file to digitize');

% Controlla se l'utente ha annullato
if isequal(filename,0)
    error('Nessun file selezionato. Operazione annullata.');
end

% Costruisci il percorso completo
infile = fullfile(pathname, filename);

% Estrai path, nome ed estensione (per il salvataggio dopo)
